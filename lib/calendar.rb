...
class Calendar
    ...
    def add(start: Date.new(2001,9,11), summary: "Twin towers", **args)
        if (args.keys - ALL_PARAMS).empty? # check if args contains no illegal arguments
            args[:end] = (start.to_time + 60*(args[:duration]-60)).to_datetime if args.has_key? :duration #to_time to add minutes (-1 hour offset)
            @cal.event do |e| # creation of the event
                e.dtstart = Icalendar::Values::DateOrDateTime.new(start).call
                e.summary = summary
                # optional args
                e.dtend = Icalendar::Values::DateOrDateTime.new(args[:end]).call if args.has_key? :end
