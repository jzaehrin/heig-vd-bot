...
# --- has method reply_to_message AND return !nil (is a reply)
        if message.respond_to?('reply_to_message') && !message.reply_to_message.nil?
            ...
            when /^\/add_event ([A-Z]+\d) SUMMARY/ # we get subject & summary
                # show kb for date
                generate_ikb(message.chat.id, "Which day ?", create_calendar_ikb(10, 2017))
            end
# --- is a CallbackQuery (care about message.data not .text)
            ...
            when /^([A-Z]+\d) kbId:(\d+)/
                ...
                # use reponse
                reponse(message.from.id, "/add_event #$1 SUMMARY", Telegram::Bot::Types::ForceReply.new(force_reply: true))
...
# --- is a classic message (care about message.text)
            ...
            when '/!set_admin'
                if @config["admin"] then
                    reponse(message.chat.id, "The admin channel has already set !")
                end
                reponse(message.chat.id, "This channel has been set to admin")
            when '/!remove_admin'
                reponse(message.chat.id, "This channel has been removed")
            when '/start_calendar'
                reponse(message.chat.id, "Hello, #{message.from.first_name}!")
            when '/add_calendar'
                reponse(message.chat.id, "TODO")
            when /^\/add_event \?/
                reponse(message.chat.id, "HOWTO : /add_event SUBJECT SUMMARY dd.mm(.aaaa) hh.mm (DURATION_MIN default: 45) (DESCRIPTION)\n")
            when /^\/add_event ([A-Z]+)/
                reponse(message.chat.id, "Add event in #$1")
            when '/add_event'
                generate_ikb(message.chat.id, "Which class subject ?", @subject.zip(@subject).each_slice(3).to_a)
            end
        end
    end
end
